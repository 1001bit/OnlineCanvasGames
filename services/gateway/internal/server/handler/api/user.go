...
func HandleUserLogin(w http.ResponseWriter, r *http.Request, userService *userservice.UserService) {
	...
	if message, ok := input.validate(); !ok {
		ServeTextMessage(w, message, http.StatusUnauthorized)
		return
	}
	...
	if err != nil {
		...
		case codes.Unauthenticated:
			ServeTextMessage(w, "Invalid username or password", http.StatusUnauthorized)
		default:
			HandleServerError(w)
		}
...
func HandleUserRegister(w http.ResponseWriter, r *http.Request, userService *userservice.UserService) {
	...
	if err != nil {
		...
		case codes.AlreadyExists:
			ServeTextMessage(w, "User with such username already exists", http.StatusUnauthorized)
		default:
			HandleServerError(w)
		}
